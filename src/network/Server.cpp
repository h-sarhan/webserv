...
Server::Server(std::string name, std::string port) : name(name), port(port)
{
	...
	if ((status = getaddrinfo(NULL, port.c_str(), &hints, &servInfo)) != 0) // first param is host IP/name and its null because we set ai_flags as AI_PASSIVE
...
void Server::bindSocket()
{
	...
	{
		if ((this->sockFd = socket(servInfo->ai_family, servInfo->ai_socktype, servInfo->ai_protocol)) == -1)
			std::cout << "socket: " << strerror(errno) << std::endl;
		else
		{
			checkErr("setsockopt", setsockopt(this->sockFd, SOL_SOCKET, SO_REUSEADDR, &reusePort, sizeof(reusePort)));
			if (bind(this->sockFd, servInfo->ai_addr, servInfo->ai_addrlen) != -1) // binding the socket to a port means - the port number will be used by the kernel to match an incoming packet to webserv's process socket descriptor. 
				break;
...
void Server::startListening()
{
	...
	newFd = checkErr("accept", accept(this->sockFd, (sockaddr *)&their_addr, &addr_size));
