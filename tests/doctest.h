...
#define DOCTEST_STATIC_ASSERT(expression, message)                             \
    ...
    {                                                                          \
        doctest::detail::static_assert_impl::StaticAssertion<                  \
            ...
            DOCTEST_CAT(                                                       \
                DOCTEST_CAT(DOCTEST_CAT(STATIC_ASSERTION_FAILED_AT_LINE_,      \
                                        __LINE__),                             \
                            _),                                                \
                message);                                                      \
    };                                                                         \
...
template <class TList, class Callable> struct ForEachType;
...
struct Types
{
  private:
    typedef
        typename Types<T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14,
...
#define DOCTEST_FORBIT_EXPRESSION(op)                                          \
    template <typename R> Expression_lhs &operator op(const R &)               \
    {                                                                          \
...
struct ExpressionDecomposer
{
    ...
    ExpressionDecomposer(assertType::Enum at) : m_assert_type(at)
    {
    }
...
template <int comparison, typename L, typename R>
DOCTEST_NOINLINE int fast_binary_assert(assertType::Enum at, const char *file,
...
template <typename L>
DOCTEST_NOINLINE int fast_unary_assert(assertType::Enum at, const char *file,
                                       int line, const char *val_str,
...
#define DOCTEST_TYPE_TO_STRING_IMPL(x)                                         \
    template <> inline const char *type_to_string<x>()                         \
    {                                                                          \
        return "<" #x ">";                                                     \
    }
...
#define DOCTEST_TEST_CASE_TEMPLATE_IMPL(decorators, T, types, anon)            \
    template <typename T> inline void anon();                                  \
    ...
    {                                                                          \
        ...
        doctest::detail::ForEachType<DOCTEST_HANDLE_BRACED_VA_ARGS(            \
                                         types)::Result,                       \
...
#define DOCTEST_IMPLEMENT_FIXTURE(der, base, func, name)                       \
    namespace                                                                  \
    {                                                                          \
    template <typename DOCTEST_UNUSED_TEMPLATE_TYPE> struct der : base         \
    {                                                                          \
        void f();                                                              \
    };                                                                         \
    }                                                                          \
...
TestCase::TestCase(funcType test, const char *file, unsigned line,
                   const TestSuite &test_suite, const char *type,
                   int template_id)
    : m_test(test), m_name(0), m_type(type),
      m_test_suite(test_suite.m_test_suite),
      m_description(test_suite.m_description), m_skip(test_suite.m_skip),
      m_may_fail(test_suite.m_may_fail),
      m_should_fail(test_suite.m_should_fail),
      m_expected_failures(test_suite.m_expected_failures),
      m_timeout(test_suite.m_timeout), m_file(file), m_line(line),
...
bool matchesAny(const char *name, const std::vector<String> &filters,
                int matchEmpty, bool caseSensitive)
{
    ...
    : m_signature(name, file, line), m_entered(false)
{
    ...
    {
        if (!matchesAny(m_signature.m_name, s->filters[6], 1,
                        s->case_sensitive))
            return;
...
void file_line_to_stream(std::ostream &s, const char *file, int line,
                         const char *tail = "")
{
    s << Color::LightGrey << fileForOutput(file)
      << (contextState->gnu_file_line ? ":" : "(")
      << (contextState->no_line_numbers
              ? 0
...
void successOrFailColoredStringToStream(std::ostream &s, bool success,
                                        assertType::Enum at,
...
std::ostream &operator<<(std::ostream &s,
                         const std::vector<IContextScope *> &contexts)
{
    if (!contexts.empty())
        s << Color::None << "  logged: ";
...
void logTestException_impl(std::ostream &s, const TestCase &tc,
                           const String &str, bool crash)
{
    file_line_to_stream(s, tc.m_file, tc.m_line, " ");
    successOrFailColoredStringToStream(
        s, false, crash ? assertType::is_require : assertType::is_check);
    s << Color::Red
      << (crash ? "test case CRASHED: " : "test case THREW exception: ")
      << Color::Cyan << str << "\n";
...
void logAssert(bool passed, const String &dec, bool threw, const String &ex,
               const char *expr, assertType::Enum at, const char *file,
               int line)
{
...
void logAssertThrows(bool threw, const char *expr, assertType::Enum at,
                     const char *file, int line)
{
...
void logAssertThrowsAs_impl(std::ostream &s, bool threw, bool threw_as,
                            const char *as, const String &ex, const char *expr,
                            assertType::Enum at, const char *file, int line)
{
    file_line_to_stream(s, file, line, " ");
    ...
    s << Color::Cyan << assertString(at) << "( " << expr << ", " << as << " ) "
      << Color::None
      << (threw ? (threw_as ? "threw as expected!"
...
void logAssertThrowsAs(bool threw, bool threw_as, const char *as,
                       const String &ex, const char *expr, assertType::Enum at,
                       const char *file, int line)
{
    logAssertThrowsAs_impl(std::cout, threw, threw_as, as, ex, expr, at, file,
                           line);
    DOCTEST_PRINT_TO_OUTPUT_WINDOW_IN_IDE_BEGIN;
...
void logAssertNothrow_impl(std::ostream &s, bool threw, const String &ex,
                           const char *expr, assertType::Enum at,
                           const char *file, int line)
{
    file_line_to_stream(s, file, line, " ");
    successOrFailColoredStringToStream(s, !threw, at);
...
ResultBuilder::ResultBuilder(assertType::Enum at, const char *file, int line,
                             const char *expr, const char *exception_type)
    : m_assert_type(at), m_file(file), m_line(line), m_expr(expr),
      m_exception_type(exception_type), m_threw(false), m_threw_as(false),
      m_failed(false)
{
#if DOCTEST_MSVC
    if (m_expr[0] ==
    ...
    if (m_failed || contextState->success)
    {
        ...
        {   //! OCLINT bitwise operator in conditional
            logAssertThrows(m_threw, m_expr, m_assert_type, m_file, m_line);
                 ...
                 assertType::is_throws_as)
        {
            logAssertThrowsAs(m_threw, m_threw_as, m_exception_type,
                              m_exception, m_expr, m_assert_type, m_file,
                              m_line);
        }
        else if (m_assert_type &   //! OCLINT bitwise operator in conditional
                 assertType::is_nothrow)
        {
            logAssertNothrow(m_threw, m_exception, m_expr, m_assert_type,
                             m_file, m_line);
        }
        else
        {
...
void MessageBuilder::log(std::ostream &s)
{
    ...
    s << getSuccessOrFailColor(false, m_severity)
      << getSuccessOrFailString(m_severity & assertType::is_warn, m_severity,
                                "MESSAGE: ");
...
bool parseOption(int argc, const char *const *argv, const char *pattern,
                 String &res, const String &defaultVal = String())
{
    res = defaultVal;
#ifndef DOCTEST_CONFIG_NO_UNPREFIXED_OPTIONS
...
void printSummary(std::ostream &s)
{
    ...
    else
    {
        ...
        s << Color::Cyan << "[doctest] " << Color::None
          << "test cases: " << std::setw(6) << p->numTestsPassingFilters
          << " | "
          << ((p->numTestsPassingFilters == 0 || anythingFailed) ? Color::None
                                                                 : Color::Green)
          << std::setw(6) << p->numTestsPassingFilters - p->numFailed
          << " passed" << Color::None << " | "
          << (p->numFailed > 0 ? Color::Red : Color::None) << std::setw(6)
          << p->numFailed << " failed" << Color::None << " | ";
        ...
        s << "\n";
        s << Color::Cyan << "[doctest] " << Color::None
          << "assertions: " << std::setw(6) << p->numAssertions << " | "
          << ((p->numAssertions == 0 || anythingFailed) ? Color::None
                                                        : Color::Green)
          << std::setw(6) << (p->numAssertions - p->numFailedAssertions)
          << " passed" << Color::None << " | "
          << (p->numFailedAssertions > 0 ? Color::Red : Color::None)
...
// execute the test
                ...
                {
                    DOCTEST_LOG_START(std::cout);
                    logTestException(*contextState->currentTest,
            ...
            if (p->duration)
                std::cout << Color::None << std::setprecision(6) << std::fixed
                          << duration << " s: " << p->currentTest->m_name
                          << "\n";
